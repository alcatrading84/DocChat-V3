    ...
    def run(self):
        try:
            if self.question == "__summarize__":
                full = self.engine.summarize(on_token=lambda t: self.token.emit(t))
...
class DocChatWindow(QMainWindow):
    def __init__(self):
        ...
        for p in ["favicon.ico", os.path.join(os.path.dirname(__file__), "..", "favicon.ico"),
                   getattr(_sys, '_MEIPASS', '') + "/favicon.ico"]:
    ...
    def _init_ui(self):
        ...
        self.btn_theme.setToolTip("Tema oscuro/claro")
        ...
        self.btn_export.setFixedWidth(34)
        self.btn_export.setToolTip("Exportar chat a TXT")
        ...
        self.btn_translate.setFixedWidth(34)
    ...
    def _summarize_docs(self):
        if self.engine.vector_store.count == 0:
            self._msg("error", "No hay documentos cargados.")
            return
        ...
        w.finished.connect(lambda r: self._msg("system", "✅ Resumen listo"))
        w.error.connect(lambda e: self._msg("error", f"Error: {e}"))
        w.start()

    def _export_chat(self):
        from PyQt6.QtWidgets import QFileDialog
        path, _ = QFileDialog.getSaveFileName(self, "Exportar chat",
            "chat_docchat.txt", "TXT (*.txt);;Todos (*)")
        if path:
            try:
                self.engine.export_chat(self.chat.toHtml(), path)
                self.sb.showMessage(f"✅ Exportado", 5000)
            except Exception as e:
    ...
    def _translate_last(self):
        ...
        if not last.strip():
            self._msg("error", "Texto vacío.")
            return
        target = "es" if self.lang == "en" else "en"
        lang_n = {"es": "español", "en": "inglés"}
        self._msg("system", f"🌐 Traduciendo a {lang_n[target]}...")
        w = StreamWorker(self.engine, f"__translate__:{target}:{last}", use_context=False)
        w.token.connect(self._on_token)
        w.finished.connect(lambda r: self._msg("system", f"✅ Traducción a {lang_n[target]} lista"))
        w.error.connect(lambda e: self._msg("error", f"Error: {e}"))
        w.start()
    ...
    def _toggle_theme(self):
        ...
        self.btn_theme.setText("🌙" if self._dark_theme else "☀️")
        bg = "#12121a" if self._dark_theme else "#f5f5f5"
        chat_bg = "#0d0d1a" if self._dark_theme else "#ffffff"
        chat_fg = "#e0e0e0" if self._dark_theme else "#222222"
        self.setStyleSheet(f"QMainWindow {{ background: {bg}; }}")
        self.chat.setStyleSheet(
            f"QTextEdit {{ background: {chat_bg}; color: {chat_fg}; "
            f"border: 1px solid #333; border-radius: 4px; padding: 8px; }}"
        )
    ...
    def _change_mode(self, idx):
        """Cambiar modo Local / Cloud."""
        mode = self.mode_sel.currentData()
        if mode == "cloud":
            from PyQt6.QtWidgets import QInputDialog
            key, ok = QInputDialog.getText(self, "☁️ OpenAI API Key",
            ...
            if ok and key.strip():
                ...
                self.sb.showMessage("☁️ Modo Cloud activado", 3000)
            else:
                self.mode_sel.setCurrentIndex(0)
                self.sb.showMessage("💻 Modo Local", 3000)
        else:
...
def main():
    ...
    # Icono de la aplicación - buscar en varias ubicaciones
    import os, sys as _sys
    icon_paths = [
        os.path.join(os.path.dirname(__file__), "..", "favicon.ico"),
        os.path.join(os.path.dirname(__file__), "..", "docchat_icon.png"),
        os.path.join(os.path.dirname(_sys.argv[0]), "favicon.ico"),
        os.path.join(_sys._MEIPASS, "favicon.ico") if hasattr(_sys, '_MEIPASS') else "",
        "favicon.ico",
        "docchat_icon.png",
    ]
    for icon_path in icon_paths:
        if icon_path and os.path.exists(icon_path):
            ...
            # También establecer para la ventana principal
            break
